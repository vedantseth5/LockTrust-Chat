...
import { addMessage as addDmMessage, addConversation, updateMessage as updateDmMessage } from '../store/dmSlice'
    ...
    onConnect: () => {
      ...
      const dmSub = client!.subscribe('/user/queue/dm', (msg) => {
        ...
        if (event.type === 'DM_NEW') {
          ...
          if (!known) store.dispatch(addConversation(conversation))
          store.dispatch(addDmMessage(message))
        } else if (event.type === 'DM_REACTION') {
          store.dispatch(updateDmMessage(event.payload))
        }
      })
  ...
  const msgSub = client.subscribe(`/topic/channel/${channelId}`, (msg) => {
    ...
    switch (event.type) {
      case 'MESSAGE_NEW': {
        const me = store.getState().auth.user
        const isMention = me
          ? new RegExp(`@${me.displayName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(?=\\s|$|[^\\w])`, 'i').test(p.content || '')
          : false
